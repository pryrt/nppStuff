# encoding=utf-8
"""https://notepad-plus-plus.org/community/topic/18134/style-token-not-saved/6

Inspired by that, and @Alan-Kilborn's https://notepad-plus-plus.org/community/topic/18052/bookmark-by-style
start experimenting with those marks
...
def main():
    ...
    # pcj
    indicator_number_list = [SCE_UNIVERSAL_FOUND_STYLE,SCE_UNIVERSAL_FOUND_STYLE_EXT1,SCE_UNIVERSAL_FOUND_STYLE_EXT2,SCE_UNIVERSAL_FOUND_STYLE_EXT3,SCE_UNIVERSAL_FOUND_STYLE_EXT4,SCE_UNIVERSAL_FOUND_STYLE_EXT5]
    clearing_not_setting = False
    console.clear()
    #/pcj
    ...
    for indic_number in indicator_number_list:
        ...
        for (styled_start_pos, styled_end_pos) in highlight_indicator_range_tups_generator(indic_number):
            ...
            # pcj: apparently, I cannot figure out how to set/fill with the right indicator .. at least, not in the right place
            console.write( "#{:02d}: pos({:04d}...{:04d}) => line({:04d}..{:04d})\n".format(indic_number, styled_start_pos, styled_end_pos, bookmark_start_line, bookmark_end_line) )
            editor.setIndicatorValue(indic_number)
            console.write("{}\n".format(editor.getIndicatorValue()))
            editor.indicatorClearRange(styled_start_pos, 3)
            sleep(1)
            editor.setIndicatorValue(indic_number)
            console.write("{}\n".format(editor.getIndicatorValue()))
            editor.indicatorFillRange(styled_start_pos, 3)
            sleep(1)
            ...
            # pcj: apparently, I cannot figure out how to set/fill with the right indicator -- at least, not in the right place...
            # this works manually:
            # editor.setIndicatorCurrent(25); editor.getIndicatorCurrent(); editor.indicatorFillRange(4800, 10)
            # this works manually:
            # editor.setIndicatorCurrent(25); editor.getIndicatorCurrent(); editor.indicatorClearRange(4800, 10)
            # ahh, experiments showed that if I set and clear in the same script, even with 1s delays, it won't show up in the visible scintilla... it only shows up if it's not undone.
